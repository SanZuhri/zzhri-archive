...
So you want to add a new collection? Good news—it's really easy now.
...
```
🚀 Creating new collection: recipes

✅ Created folder: src/content/recipes/
✅ Created folder: src/content/recipes/_assets/
✅ Created file: src/content/recipes/_example.md
✅ Updated: src/collections.config.ts
✅ Types generated successfully

✨ Collection "recipes" created successfully!
```
...
Then check:
- `http://localhost:4321/collections/` — your collection should be listed
- `http://localhost:4321/collections/recipes/` — collection page
- `http://localhost:4321/collections/recipes/chocolate-cake/` — your post

## What each property means

When you add a collection, here's what each field does:

- **`type`** — Set to `'collection'` for regular collections
- **`title`** — Display name (like "Recipes")
- **`description`** — Short description for the collections page
- **`urlPattern`** — URL structure (must include `{slug}`)
- **`listPage`** — Where the collection list lives
- **`backLabel`** — Text for the back button
- **`showInCollectionsList`** — Show it on `/collections/` or not

Keep collection names lowercase with no spaces. Keep descriptions under 50 characters.

## Why this is better than before

**The old way** meant editing 5+ files every time you added a collection. Config file, routing files, type definitions—all manual.
